...
{
	//-w=4  -h=11 -s=14 stereo_calib.xml
...
StereoCalib(const vector<Mat> &imagelist, Size boardSize, float squareSize, bool displayCorners = false, bool useCalibrated = true, bool showRectified = true)
{
	...
	for (i = j = 0; i < nimages; i++)
	{
		for (k = 0; k < 2; k++)
		{
			...
			{
				cout << "The image " << i << " has the size different from the first image size. Skipping the pair\n";
				break;
			}
			bool found = false;
			...
			for (int scale = 1; scale <= maxScale; scale++)
			{
				...
				found = findCirclesGrid(timg, boardSize, corners, cv::CALIB_CB_ASYMMETRIC_GRID | cv::CALIB_CB_CLUSTERING, blobDetector);
...
int main(int argc, char **argv)
{
	Size boardSize;
	bool showRectified;
	cv::CommandLineParser parser(argc, argv, "{w|4|}{h|11|}{s|14.0|}{nr||}{help||}");
	if (parser.has("help"))
		return print_help();
	...
	int b = 1;
	// 左右的对
	vector<Mat> image_list(30);
	image_list.clear();
	int count = 0;
	cout << "1 继续, 0 退出 , 最多30张" << endl;
	while (cin >> b, b) {
		Fine_Tune_Device.GrabImageDoubleCamera();
		image_list.push_back(Fine_Tune_Device.left_img);
		image_list.push_back(Fine_Tune_Device.right_img);
	...
	//StereoCalib(image_list, boardSize, squareSize, true, true, showRectified);
	return 0;
}
